...
import {getAnswerTemplate, resultAndBecauseTemplate} from "@/template/commonTemplate";
...
const botGames: BotGames = {
  ...
  '猜字谜': async (message: Message, text: string) => {
    ...
    await message.say(getAnswerTemplate(zimiResult.content.content, answerTime));
    setTimeout(async () => {
      await message.say(resultAndBecauseTemplate(zimiResult.answer, zimiResult.reason));
    }, answerTime)
  }
}
